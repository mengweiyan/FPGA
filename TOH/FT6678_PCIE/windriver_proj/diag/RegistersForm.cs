...
namespace Jungo.ft6678_yolo_diag
{
    public class RegistersForm : System.Windows.Forms.Form
    {
        ...
        public RegistersForm(FT6678_YOLO_Device dev, ACTION_TYPE regType)
        {
            InitializeComponent();
            ...
            case ACTION_TYPE.CFG:
                {
                    ...
                    WDC_REG[] regs = dev.Regs.gFT6678_YOLO_CfgRegs;
            ...
            case ACTION_TYPE.RT:
                {
                    ...
                    cmboRegs.Visible = true;
                    WDC_REG[] regs = m_device.Regs.gFT6678_YOLO_RT_Regs;
                    for (int i = 0; i < regs.GetLength(0); ++i)
                    {
                        cmboRegs.Items.AddRange(new object[]{regs[i].sName +
                            " size: " + regs[i].dwSize.ToString("X") +
        ...
        private void TranslateInput_Regs()
        {
            if (m_direction != RW.READ_ALL)
            {
                ...
                m_regs[0] = (m_regType == ACTION_TYPE.CFG) ?
                    m_device.Regs.gFT6678_YOLO_CfgRegs[iSelectedReg] :
                    m_device.Regs.gFT6678_YOLO_RT_Regs[iSelectedReg];
            ...
            else
            {
                m_regs = (m_regType == ACTION_TYPE.CFG) ?
                    m_device.Regs.gFT6678_YOLO_CfgRegs :
                    m_device.Regs.gFT6678_YOLO_RT_Regs;
            }
        }
